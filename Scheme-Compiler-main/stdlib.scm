...
(define fold-left
  ...
  (letrec ((fold_left_loop (lambda (f acc list_cur)
							(if (null? list_cur) 
								acc 
								(fold_left_loop f (f acc (car list_cur)) (cdr list_cur))))))
			fold_left_loop)))


(define fold-right
	(let ((null? null?) (car car) (cdr cdr))
		(letrec ((fold_right_loop (lambda (f acc list_cur)
								(if (null? list_cur)
									acc
									(f (car list_cur) (fold_right_loop f acc (cdr list_cur)))))))
									fold_right_loop)))
...
(define cons*
	(let ((null? null?) 
		...
		(apply apply)
		(app app))
	(letrec ((cons_iter_loop 
			(lambda list_cur 
				(cond ((null? list_cur) '())
					((null? (cdr list_cur)) (car list_cur))
					(else (cons (car list_cur) (apply cons_iter_loop (cdr list_cur))))))))
	cons_iter_loop)))
